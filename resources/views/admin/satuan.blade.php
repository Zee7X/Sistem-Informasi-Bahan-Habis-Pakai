                ...
                <tr class="hover:bg-blue-50">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-center text-sm">
                        <div class="flex justify-center space-x-2">
                            <button @click="$dispatch('open-edit-modal', {{ $item->id }})" type="button" class="p-2 bg-blue-100 rounded-lg shadow hover:bg-blue-200 transition">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-blue-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.477 0 8.268 2.943 9.542 7-1.274 4.057-5.065 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                </svg>
                            </button>
                            <button @click="$dispatch('open-delete-modal', {{ $item->id }})" type="button" class="p-2 bg-red-100 rounded-lg shadow hover:bg-red-200 transition">
...
<div x-data="{ open: false, deleteId: null }"
    @open-delete-modal.window="open = true; deleteId = $event.detail"
    x-show="open" x-cloak
    class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white rounded-xl shadow-lg w-96 p-6">
        <h2 class="text-lg font-bold mb-4">Konfirmasi Hapus</h2>
        <p class="mb-4">Apakah kamu yakin ingin menghapus Satuan ini?</p>
