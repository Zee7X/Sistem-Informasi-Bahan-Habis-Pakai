...
<div x-data="{ sidebarOpen: false }">
    ...
    <aside :class="{'translate-x-0': sidebarOpen, '-translate-x-full': !sidebarOpen}" 
        ...
        <div x-data="{ open: {{ in_array($currentRoute, ['bahan','admin.satuan.index']) ? 'true' : 'false' }} }" class="flex flex-col mt-2">
            <button @click="open = !open"
                    class="flex items-center justify-between px-3 py-2 text-gray-700 font-medium transition rounded-xl shadow-sm hover:shadow-md
                    {{ in_array($currentRoute, ['bahan','admin.satuan.index']) ? 'text-blue-700 bg-blue-50 shadow-md' : 'hover:text-blue-700 bg-white' }}
                    focus:outline-none">
                <div class="flex items-center space-x-2">
            ...
            <div x-show="open" x-transition x-cloak class="mt-1 ml-6 flex flex-col space-y-1">
                ...
                <a href="{{ route('admin.satuan.index') }}" 
                    class="px-3 py-1 flex items-center space-x-2 text-gray-700 text-sm font-medium transition rounded-xl shadow-sm hover:shadow-md
                    {{ $currentRoute == 'admin.satuan.index' ? 'text-blue-700 bg-blue-50 shadow-md' : 'hover:text-blue-700 bg-white' }}">
